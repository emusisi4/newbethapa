...
class AuthorisedbranchanduserController extends Controller
{
    /**
    ...
    return    Branchanduser::with(['branchanduserBranchname','branchandUsername'])->latest('id')
   // return   Branchanduser::latest('id')
     ->where('username', $roleto)
        ->paginate(80);
      }
...
DB::table('branchesandmachines')->where('branchname', $bn)->where('machinename', $prod)->delete();
       ...
       return Branchanduser::Create([
    

      'branchname' => $request['product'],
     'username'=> $request['branch'],
...
$user->update($request->all());
    ...
    public function destroy($id)
    {
     ...
     //   $this->authorize('isAdmin'); 
        ...
        $user = Branchanduser::findOrFail($id);
        $user->delete();
