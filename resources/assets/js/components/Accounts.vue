...
<div  v-if="gencomponentaccessCahtransactions > 0 "> 
        ...
        <!-- Start of the -->
 <div class="bethapa-component-header" >SYSTEM ACCOUNTS </div>        
<div class="col-12 col-sm-12 col-lg-12">
            <div class="card card-primary card-outline card-tabs">
              <div class="card-header p-0 pt-1 border-bottom-0">
                <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                  <!--   -->
                  <li class="nav-item" v-if="useraccountbalancesSettings > 0 " >
                    <a class="nav-link active" id="custom-tabs-two-home-tab"
                     data-toggle="pill" href="#custom-tabs-two-home" role="tab"
                     @click="loadUseraccountbalances()" aria-controls="custom-tabs-two-home" aria-selected="true">USER ACCOUNT BALANCE</a>
                  </li>
<!--  -->


                  <li class="nav-item" v-if="branchraccountbalancesSettings > 0 " > 
                    <a class="nav-link" id="custom-tabs-two-profile-tab"
                     data-toggle="pill" href="#custom-tabs-two-profile" role="tab"
                     @click="loadBranchaccountbalances()" aria-controls="custom-tabs-two-profile" aria-selected="false">BRANCH ACCOUNT BALANCE</a>
                  </li>

 

                  <li class="nav-item" v-if="branchpayoutaccessSettings > 0 " >
                    <a class="nav-link" id="custom-tabs-two-messages-tab"
                    @click="loadBranchpayouts()" data-toggle="pill" href="#custom-tabs-two-messages" role="tab" 
                    aria-controls="custom-tabs-two-messages" aria-selected="false">BRANCH PAYOUT</a>
                  </li>
                 
                  <li class="nav-item">
                    <a class="nav-link" id="custom-tabs-two-settings-tab" data-toggle="pill"
                     href="#custom-tabs-two-settings" role="tab" @click="loadCashInTransfers()"
                     aria-controls="custom-tabs-two-settings" aria-selected="false">CASH-IN TRANSFERS</a>
                  </li>
<!--v-if="shopbalancingaccessSettings > 0 "  -->
                   <li class="nav-item" >
                    <a class="nav-link" id="custom-tabs-two-three-tab" data-toggle="pill"
                     href="#custom-tabs-two-three" role="tab" @click="loadCashoutTransfers()" 
                     aria-controls="custom-tabs-two-three" aria-selected="false">CASH-OUT TRANSFERS</a>
                  </li>

<!--v-if="cashcollectionaccessSetting > 0 "  -->
                    <li class="nav-item"  >
      ...
      <div class="tab-content" id="custom-tabs-two-tabContent"  >
                  ...
                  <!-- tab one start -->
                 <div class="tab-pane fade show active" id="custom-tabs-two-home" role="tabpanel" v-if="useraccountbalancesSettings > 0 " aria-labelledby="custom-tabs-two-home-tab"> 
               
                 <div class="bethapa-table-header">
                      USERS AND ACCOUNT BALANCES 
                      <!-- <button type="button" v-if="allowedtoaddpauout > 0 " class="add-newm" @click="newBranchmodal" >Add New </button>  -->
                     </div>
                  ...
                  <tbody>
                    ...
                    <tr v-for="uaccount in useraccountbalancerecords.data" :key="uaccount.id">
                      
                    <td>{{uaccount.id}}</td>
                     <td> <template v-if="uaccount.username_balance">	{{uaccount.username_balance.name}}</template></td>
                       <td>{{currencydetails}} {{formatPrice(uaccount.amount)}}</td>
                          ...
                          <td>
                      {{(uaccount.updated_at)}}</td>
                     
                                

                       <td> <template v-if="uaccount.branch_name">	{{uaccount.branch_name.branchname}}</template></td>
    ...
    <div class="card-footer">
                <ul class="pagination pagination-sm m-0 float-right">
                   <pagination :data="useraccountbalancerecords" @pagination-change-page="paginationResultsBranches"></pagination>
                </ul>
              </div>
...
<!--  -->
   ...
   <table class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr v-for="branchaccount in branchescashoutstandingrecords.data" :key="branchaccount.id">
                      
                    <td>{{branchaccount.id}}</td>
                     <td> <template v-if="branchaccount.branch_balance">	{{branchaccount.branch_balance.branchname}}</template></td>
                       <td>{{currencydetails}} {{formatPrice(branchaccount.outstanding)}}</td>
                          ...
                          <td>
                      {{(branchaccount.updated_at)}}</td>
                     
                                

                       <td> <template v-if="branchaccount.branch_name">	{{branchaccount.branch_name.branchname}}</template></td>
...
<!-- mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm -->
 ...
 <form @submit.prevent="SaveRoletoaddmainmenu()">
              ...
              <div class="bethapa-table-header">
                    BRANCH PAYOUT DETAILS
                      <!-- <button type="button"  class="add-newm" @click="newBranchpayoutbranch" >Add New </button> -->
                        <button type="button" v-if="allowedtoaddpauout > 0" class="add-newm" @click="newBranchpayoutbranch" >Pay Ticket / Slip</button>
                     </div>
              ...
              <table class="table table-striped">
                 ...
                 <tr v-for="allpayoutrecords in branchpayoutrecords.data" :key="allpayoutrecords.id">
                    
                 <td>{{allpayoutrecords.receiptno}}</td>
                 <td>    <template v-if="allpayoutrecords.expense_typeconnect">	{{allpayoutrecords.expense_typeconnect.branchname}}</template></td>

                 <td>{{allpayoutrecords.datemade}}</td>

                 <td>    <template v-if="allpayoutrecords.expense_category">	{{allpayoutrecords.expense_category.branchname}}</template></td>
                                              
                                                
                 <td>{{ (currencydetails) }}  {{formatPrice(allpayoutrecords.amount)}}</td>
                 <td>{{allpayoutrecords.datepaid}}</td>
                 <td>    <template v-if="allpayoutrecords.paying_userdetails">	{{allpayoutrecords.paying_userdetails.name}}</template></td>
                
                 <td>{{allpayoutrecords.created_at}}</td>
                           
                <td>
                            <button type="button" v-if="allowedtoeditpayout > 0 "   class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="editPayout(allpayoutrecords)">Edit</button>
                             <button type="button" v-if="allowedtodeletepayout > 0 " class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deletepayout(allpayoutrecords.id)"> DEl </button>
...
<!-- mmmmmmmmmmmmmmmmmmmmmmmmmmm -->
 ...
 <form @submit.prevent="SaveRoletoaddmainmenu()">
              ...
              <div class="bethapa-table-header">
                    MY CASH-IN TRANSFER DETAILS
                      <!-- <button type="button" v-if="allowedtoaddnewexpensereccord > 0"  class="add-newm" @click="newBranchpayoutbranch" >Add New </button> -->
                        <!-- <button type="button"  class="add-newm" @click="cashTransfer" >Transfer Cash</button> -->
                     </div>
             ...
             <table class="table table-bordered table-striped">
               ...
               <th>#</th>
                  ...
                  <tbody>
                    ...
                    <tr v-for="allcashinAdmindet in cashtransferadmincashintransferrecords.data" :key="allcashinAdmindet.id">
                    
                    <td>{{allcashinAdmindet.id}}</td>
                  
                    <td>{{allcashinAdmindet.transerdate}}</td>
                           <td>    <template v-if="allcashinAdmindet.username_transferfrom">	{{allcashinAdmindet.username_transferfrom.name}}</template></td>
                            <td>    <template v-if="allcashinAdmindet.username_transferto">	{{allcashinAdmindet.username_transferto.name}}</template></td>
                          <td>{{allcashinAdmindet.description}}</td>
                       
                               <td> {{ (currencydetails) }} {{formatPrice((allcashinAdmindet.amount))}}</td>
                               <td>
                                 
                                 <div v-if=" allcashinAdmindet.status == 0 ">
                            <span class="cell" style="color:maroon ;">  
                      
                   </span> 
                    <button v-show="allcashinAdmindet.status < 1"
                     type="button"   class="btn  bg-gradient-danger btn-xs"
                       @click="completeCashtransfer(allcashinAdmindet.id)"> PENDING : Click here to confirm  </button>
                     
                                 </div>
                                     <button v-show="allcashinAdmindet.status == 1" type="button"   class="btn  bg-gradient-success btn-xs"  > Completed  </button>
                                 </td>
...
<!-- Modal add menu -->
<div class="modal fade" id="addnewcashtransfermodal">
        <div class="modal-dialog modal-dialog-top modal-lg">
        <div  class="modal-content">
            <div  class="modal-header">
                <h4  v-show="!editmode"    class="modal-title">CASH TRANSFER FORM</h4> 
                <h4  v-show="editmode" class="modal-title" >UPDATE RECORD</h4> 
                <button  type="button" data-dismiss="modal" aria-label="Close" class="close"><span  aria-hidden="true">×</span></button></div> 
               
               
               
                <form class="form-horizontal" @submit.prevent="editmode ? updateadmincashintransfer():createNewcashTransfer()"> 
                 ...
                 <div class="form-group row">
                            ...
                            <label class="col-sm-2 col-form-label">Transfer To   </label>
                              <div class="col-sm-6">
                        <select name ="transferto" v-model="form.transferto" id ="transferto" class="form-control" :class="{'is-invalid': form.errors.has('transferto')}">
<option value=" ">  </option>
<option v-for='data in authorisedtransferwallets' v-bind:value='data.id'>{{ data.name }}</option>
      ...
      <has-error :form="form" field="amount"></has-error>
                        ...
                        <div class="form-group row">
                            ...
                            <label class="col-sm-2 col-form-label">Date </label>
                              <div class="col-sm-6">
                                 <input v-model="form.transerdate" type="date" name="transerdate"
                       class="form-control" :class="{ 'is-invalid': form.errors.has('transerdate') }">
                       <has-error :form="form" field="transerdate"></has-error>
                              </div>
...
<!-- mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm -->
            ...
            <table class="table table-bordered table-striped">
               ...
               <th>#</th>
                  ...
                  <tbody>
                    ...
                    <tr v-for="allcashoutAdmindet in cashtransferadmincashouttransferrecords.data" :key="allcashoutAdmindet.id">
                    
                    <td>{{allcashoutAdmindet.id}}</td>
                  
                    <td>{{allcashoutAdmindet.transerdate}}</td>
                           <td>    <template v-if="allcashoutAdmindet.username_transferfrom">	{{allcashoutAdmindet.username_transferfrom.name}}</template></td>
                            <td>    <template v-if="allcashoutAdmindet.username_transferto">	{{allcashoutAdmindet.username_transferto.name}}</template></td>
                          <td>{{allcashoutAdmindet.description}}</td>
                       
                               <td> {{ (currencydetails) }} {{formatPrice((allcashoutAdmindet.amount))}}</td>
                               <td>
                                 
                                 <div v-if=" allcashoutAdmindet.status == 0 ">
                            <span class="cell" style="color:maroon ;">  
                      
                   </span> 
                    <button v-show="allcashoutAdmindet.status < 1"
                     type="button"   class="btn  bg-gradient-danger btn-xs"
                     > PENDING  </button>
           ...
           <button v-show="allcashoutAdmindet.status == 1" type="button"   class="btn  bg-gradient-success btn-xs"  > Completed  </button>
                                 </td>
      ...
      <button type="button" v-show="allcashoutAdmindet.status == 0" v-if="allowedtoeditcashouttransferrecord > 0 "   class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="editCashtransferrec(allcashoutAdmindet)">Edit</button>
      <button type="button" v-show="allcashoutAdmindet.status == 0"  v-if="allowedtodeletecashouttransferrecord > 0 " class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deletecashtransfer(allcashoutAdmindet.id)"> DEl </button>
...
<!-- End od pane -->
      ...
      <!-- <button type="button" v-if="allowedtoeditadmincashcollection > 0 "   class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="editCashtransferrec(alladmincashcollection)">Edit</button>
...
<!-- End of pane -->
      ...
      <button type="button" v-if="allowedtodeleteadmincashcredit> 0 " class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deletecashtransfer(alladmincashouts.id)"> DEl </button>
...
<script>
    export default {
        ...
        return {
         ...
         gencomponentaccessCahtransactions:'',
         useraccountbalancesSettings:'',
           
         branchraccountbalancesSettings:'',
     ...
     ///    
       ...
       ////////////////////////////////////////////////////////////////////////////////////
          ...
          editmode: false,
          useraccountbalancerecords : {},
          branchescashoutstandingrecords:{},
          datarecordsSubmenusauthorised:{},
          allowedrolecomponentsObject :{},
          shopbalancingdatarecords :{},
           cashtransferadmincashintransferrecords :{},
           cashtransferadmincashouttransferrecords :{},
          ...
          allowedtoaddshopBalancingRecord:{},
          ...
          allowedtodeletepayout:{},
          allowedtoeditpayout:{},
          allowedtoeditexpenserecord:{},
          allowedtoeditcashouttransferrecord:{},
          allowedtodeletecashouttransferrecord:{},
          allowedtoeditadmincashcollection:{},
           allowedtoeditadmincashcredit:{},
           allowedtoaddnewcashcredit:{},
...
SavetheCollectionbranch (){
   ...
   axios.get("api/branchcashbalance").then(({ data }) => (this.admincashindatarecords = data));
...
myClickEventdddd($event) { const elem = this.$refs.myBtn89
            elem.click()
        },
...
SaveRoletoaddmainmenu (){ 
    this.form.post('api/roletoaddmainmenu');
      axios.get("api/payouts").then(({ data }) => (this.branchpayoutrecords = data));
            },
...
paginationroleAuthorisedsubmenues(page = 1) {
                      ...
                      paginationroleAuthorisedmainmenus(page = 1) {
                        axios.get('api/authorisedmainmenus?page=' + page)
                          .then(response => {
                            this.branchpayoutrecords = response.data;
                          });
                      },
...
paginationroleAuthorisedcomponentsfeature(page = 1) {
                        axios.get('api/shopcashoutdetails?page=' + page)
                          .then(response => {
   ...
   paginationResultsBranches(page = 1) {
                        axios.get('api/shopcashoutdetails?page=' + page)
                          .then(response => {
                            this.useraccountbalancerecords = response.data;
                          });
                      },
...
//   },
  ...
  loadBranchaccountbalances(){
       
       axios.get("api/branchcashbalance").then(({ data }) => (this.branchescashoutstandingrecords = data));
     //  this.getRoles();
     //  this.getUsertypes();
     ...
     axios.get("api/todayspayouttotal").then(({ data }) => (this.todayspayouttotal = data));
        ...
        axios.get("api/gencomponentaccessCahtransactions").then(({ data }) => (this.gencomponentaccessCahtransactions = data));
        axios.get("api/useraccountbalancesSettings").then(({ data }) => (this.useraccountbalancesSettings = data));
        axios.get("api/branchraccountbalancesSettings").then(({ data }) => (this.branchraccountbalancesSettings = data));
 ...
 axios.get("api/getcurrencydetails").then(({ data }) => (this.currencydetails = data));
  ...
  loadUseraccountbalances(){
       ...
       axios.get("api/thuusersaccountbalance").then(({ data }) => (this.useraccountbalancerecords = data));
     //  this.getRoles();
     //  this.getUsertypes();
     //   this.checkBranchescomponentfeatures();
        axios.get("api/gencomponentaccessCahtransactions").then(({ data }) => (this.gencomponentaccessCahtransactions = data));
...
loadCashoutTransfers(){
        axios.get("api/maycashoutdetails").then(({ data }) => (this.cashtransferadmincashouttransferrecords = data));
        
        
          axios.get("api/getExpensestomake").then(({ data }) => (this.expenseslist = data));
           
           axios.get("api/getauthorisedtransferlist").then(({ data }) => (this.authorisedtransferwallets = data));
       
       
              axios.get("api/allowedtoeditcashouttransferrecord").then(({ data }) => (this.allowedtoeditcashouttransferrecord = data));
              axios.get("api/allowedtodeletecashouttransferrecord").then(({ data }) => (this.allowedtodeletecashouttransferrecord = data));
              axios.get("api/allowedtomakecashtransfer").then(({ data }) => (this.allowedtomakecashtransfer = data));
...
loadCashInTransfers(){
        axios.get("api/mycashindetails").then(({ data }) => (this.cashtransferadmincashintransferrecords = data));
          ...
          axios.get("api/getExpensestomake").then(({ data }) => (this.expenseslist = data));
           ...
           axios.get("api/getauthorisedtransferlist").then(({ data }) => (this.authorisedtransferwallets = data));
              ...
              axios.get("api/geteditbranchexpenserecord").then(({ data }) => (this.allowedtoeditexpenserecord = data));
              axios.get("api/deletebranchexpenserecord").then(({ data }) => (this.allowedtodeleteexpenserecord = data));
              axios.get("api/getAddnewexpenserecord").then(({ data }) => (this.allowedtoaddnewexpensereccord = data));
...
axios.get("api/payouts").then(({ data }) => (this.branchpayoutrecords = data));
 ...
 loadAdmincashout(){
     axios.get("api/cashoutfromoffice").then(({ data }) => (this.admincashoutrecords = data));
          axios.get("api/getaddnewcashcredit").then(({ data }) => (this.allowedtoaddnewcashcredit = data));                   
       axios.get("api/geteditcashcredit").then(({ data }) => (this.allowedtoeditadmincashcredit = data));
       axios.get("api/getdeletecashcredit").then(({ data }) => (this.allowedtodeleteadmincashcredit = data));
     
  this.SavetheCreditbranch();
...
cashTransfer(){
     ...
     $('#addnewcashtransfermodal').modal('show');
            },
newBranchpayoutbranch(){
        this.editmode = false;
        this.form.clear();
        this.form.reset();
...
newCashcollection(){
   ...
   editPayout(branchpayoutrecords){
                this.editmode = true;
                 this.form.clear();
        this.form.reset();
        this.form.fill(branchpayoutrecords);

$('#addnewpayoutbranch').modal('show');
            },

 editCashtransferrec(cashtransferadmincashintransferrecords){
                this.editmode = true;
                 this.form.clear();
        this.form.reset();
        this.form.fill(cashtransferadmincashintransferrecords);
...
if (result.isConfirmed) {
  this.form.delete('api/branchcashbalance/'+id).then(()=>{
...
///////////////////////
   ...
   editcashcredit(admincashoutrecords){
                this.editmode = true;
                 this.form.clear();
        this.form.reset();
        this.form.fill(admincashoutrecords);
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/cashoutfromoffice/'+id).then(()=>{
...
updateadmincashintransfer(){
    this.$Progress.start();
          
this.form.put('api/mycashindetails/'+this.form.id)
  .then(()=> {
    // on success
   $('#addnewcashtransfermodal').modal('hide');
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/mycashindetails/'+id).then(()=>{
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/approvecashtransfre/'+id).then(()=>{
...
/// send request ti
if (result.isConfirmed) {
  this.form.delete('api/approvecashin/'+id).then(()=>{
...
createNewcashTransfer(){
      this.$Progress.start();
        this.form.post('api/mycashindetails')
        .then(()=>{
...
axios.get("api/authorisedcomponents").then(({ data }) => (this.allowedrolecomponentsObject = data));
    ...
    $('#addnewshopbalancingrecord').modal('hide');
 //   Fire.$emit('AfterAction');
  Toast.fire({
  icon: 'success',
  title: 'Component Authorised'
...
if (result.isConfirmed) {
 ...
 checkBranchescomponentfeatures: function(){
              axios.get("api/getAddnewpayout").then(({ data }) => (this.allowedtoaddpauout = data));
              //axios.get("api/getviewbranch").then(({ data }) => (this.allowedtoviewbranch = data));
              
              axios.get("api/geteditbranchpayout").then(({ data }) => (this.allowedtoeditpayout = data));
              axios.get("api/deletebranchpayout").then(({ data }) => (this.allowedtodeletepayout = data));
           },
 ...
 checkSubmenuaccessfeatures: function(){
           ...
           checkComponentaccessfeatures: function(){
             ...
             axios.get("api/getRevokecomponentaccess").then(({ data }) => (this.allowedtorevokecomponentaccess = data));
              ...
              axios.get("api/allowedtoaddshopBalancingRecord").then(({ data }) => (this.allowedtoaddshopBalancingRecord = data));
              axios.get("api/allowedtodeleteshopBalancingRecord").then(({ data }) => (this.allowedtodeleteshopBalancingRecord = data));
              axios.get("api/allowedtoviewshopBalancingRecord").then(({ data }) => (this.allowedtoviewshopBalancingRecord = data));
...
///////////////////////////////////////////////////
  ...
  //setInterval(() =>this.loadUseraccountbalances(),3000);
        }
    }
</script>
