                ...
                <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                 ...
                 <li class="nav-item">
                    ...
                    data-toggle="pill" href="#custom-tabs-two-eight" role="tab"
                     aria-controls="custom-tabs-two-eight" aria-selected="false">Fish Reset Codes</a>
                  </li>
...
<!-- 9999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999 -->
  <div class="tab-pane fade" id="custom-tabs-two-eight" role="tabpanel" aria-labelledby="custom-tabs-two-eight-tab">
              ...
              <div class="bethapa-table-header">
                      ...
                      MACHINE CODES / ADMIN  <button type="button" class="add-newm" @click="newbranchmachine" >Add New </button>
                     </div>
<!--  v-if="allowedtogivecomponentaccess > 0" -->
   <table class="table table-bordered table-striped">
                  <thead>
                  ...
                  <tbody>
                    <tr>
                                       ...
                                       <tr v-for="allbrmac in branchmachinecodesdatarecords.data" :key="allbrmac.id">
                                ...
                                <td>{{allbrmac.created_at | myDate }}</td>  
                                  ...
                                  <td> <template v-if="allbrmac.machinecodereset_branchname">	{{allbrmac.machinecodereset_branchname.branchname}}</template></td>

                                  <td> <template v-if="allbrmac.machinecodereset_machinename">	{{allbrmac.machinecodereset_machinename.machinename}}</template></td>
   <td>{{allbrmac.machinecode }}</td>  
...
<!--  v-if="allowedtoedituser > 0 "  -->
  <button type="button"  class="btn  bg-gradient-secondary btn-xs fas fa-edit"  @click="activatecode(branchmachinecodesdatarecords)">Edit</button>
                             <!--  v-if="allowedtodeleteuser > 0 " -->
<button type="button" class="btn  bg-gradient-danger btn-xs fas fa-trash-alt" @click="deleteUser(branchmachinecodesdatarecords.id)"> DEl </button>
...
<div class="modal fade" id="addnewbranchmachine">
        ...
        <div  class="modal-content">
            <div  class="modal-header">
                <h4  v-show="!editmode"    class="modal-title">Machine Codes</h4> 
                <h4  v-show="editmode" class="modal-title" >UPDATE RECORD</h4> 
                <button  type="button" data-dismiss="modal" aria-label="Close" class="close"><span  aria-hidden="true">×</span></button></div> 
...
paginationroleAuthorisedcomponentsfeature(page = 1) {
  ...
  loadAuthorisedbranchmachines(){
    axios.get("api/newfishcurrentcodes").then(({ data }) => (this.branchmachinecodesdatarecords = data));
...
createnewBranchmachinecode(){
      this.$Progress.start();
        this.form.post('api/newfishcurrentcodes')
        .then(()=>{
